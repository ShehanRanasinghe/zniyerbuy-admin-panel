...
const initialProducts = [
  { id: 1, name: "Samba Rice 5kg", shop: "Mahinda Fresh", category: "Rice & Grains", price: "LKR 1,450", status: "Active", initials: "SR", color: "#1a3a2a", textColor: "#1d9e75" },
  ...
  { id: 3, name: "Green Tomatoes 1kg", shop: "Dambulla Veggies", category: "Vegetables", price: "LKR 220", status: "Active", initials: "GT", color: "#1a2a3a", textColor: "#6b9aaa" },
  { id: 4, name: "Anchor Milk Powder 400g", shop: "Mahinda Fresh", category: "Dairy", price: "LKR 1,890", status: "Deal", initials: "AM", color: "#2a2a10", textColor: "#f0a500" },
  { id: 5, name: "Kurakkan Flour 1kg", shop: "Dambulla Veggies", category: "Rice & Grains", price: "LKR 560", status: "Active", initials: "KC", color: "#1a3a2a", textColor: "#1d9e75" },
  { id: 6, name: "Coconut Oil 500ml", shop: "Spice Garden", category: "Cooking Oils", price: "LKR 760", status: "Active", initials: "CO", color: "#1a2a3a", textColor: "#6b9aaa" },
  { id: 7, name: "Dhal 1kg", shop: "Mahinda Fresh", category: "Rice & Grains", price: "LKR 420", status: "Deal", initials: "DH", color: "#2a2a10", textColor: "#f0a500" },
  { id: 8, name: "Gotukola Bundles", shop: "Dambulla Veggies", category: "Vegetables", price: "LKR 60", status: "Active", initials: "GK", color: "#1a3a2a", textColor: "#1d9e75" },
];

const statusBadge: Record<string, string> = {
  Active: "bg-[#1a3a2a] text-[#1d9e75] border border-[#1d9e75]",
  Flagged: "bg-[#2a1a1a] text-[#e24b4a] border border-[#e24b4a]",
  Deal: "bg-[#2a2a10] text-[#f0a500] border border-[#f0a500]",
};
  ...
  return (
    <div className="min-h-screen bg-[#0d2128] p-6">
      ...
      <div className="mb-6">
        ...
        <p className="text-[#6b9aaa] text-sm mt-1">Monitor all products and active deals on the platform</p>
      </div>

      {/* Tabs */}
      <div className="flex gap-1 mb-6 bg-[#112a33] border border-[#1a4a5a] rounded-xl p-1 w-fit">
      ...
      <div className="grid grid-cols-4 gap-3 mb-6">
        <div className="bg-[#112a33] border border-[#1a4a5a] rounded-xl p-4">
          ...
          <div className="text-[#6b9aaa] text-xs mt-1">Total products</div>
        </div>
        <div className="bg-[#112a33] border border-[#1a4a5a] rounded-xl p-4">
          <div className="text-xl font-medium text-[#1d9e75]">{products.filter((p) => p.status === "Active").length}</div>
        ...
        <div className="bg-[#112a33] border border-[#1a4a5a] rounded-xl p-4">
          ...
          <div className="text-[#6b9aaa] text-xs mt-1">Flagged</div>
        </div>
        <div className="bg-[#112a33] border border-[#1a4a5a] rounded-xl p-4">
          <div className="text-xl font-medium text-[#f0a500]">{products.filter((p) => p.status === "Deal").length}</div>
          <div className="text-[#6b9aaa] text-xs mt-1">Active deals</div>
        </div>
      </div>
        ...
        <table className="w-full border-collapse">
          <thead>
            <tr className="bg-[#0d2128]">
              <th className="text-left px-4 py-3 text-xs text-[#4a7a8a] font-medium uppercase tracking-wider">Product</th>
              <th className="text-left px-4 py-3 text-xs text-[#4a7a8a] font-medium uppercase tracking-wider">Category</th>
              <th className="text-left px-4 py-3 text-xs text-[#4a7a8a] font-medium uppercase tracking-wider">Price</th>
          ...
          <tbody>
            {filtered.map((product) => (
              <tr key={product.id} className="border-t border-[#0d2128] hover:bg-[#0f2530] transition-colors">
                <td className="px-4 py-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-[#4a7a8a]">{product.shop}</div>
                    </div>
                  </div>
                </td>
                <td className="px-4 py-3 text-xs text-[#6b9aaa]">{product.category}</td>
                ...
                <td className="px-4 py-3 flex items-center gap-2">
                  {product.status === "Flagged" ? (
                    <button
                      ...
                      className="text-xs text-[#6b9aaa] border border-[#1a4a5a] rounded-md px-2 py-1 hover:bg-[#112a33] transition-colors"
                    >
                      Unflag
                    </button>
                  ) : (
            ...
            {filtered.length === 0 && (
              <tr>
                <td colSpan={5} className="px-4 py-8 text-center text-sm text-[#4a7a8a]">No products found</td>
              </tr>
            )}
          </tbody>
        </table>
